# Sonar Ranging calculation: find time & distance from Tx pulse to Rx1,2 echoes
# v0.62 J.Beale 14-Sep-2020
...
#}
 ...
 # -------------------------------------------------------------------
 # --- search for Rx1 peak (coupler reflection)
 sl2 = idx(1) + int32(p.t2 * fsr);  # delay after Tx peak to look for Rx1,2
 sl2a = idx(1) + int32(p.t2a * fsr); # Rx1 won't be found beyond this
 w2a = -w1(sl2:sl2a);  # Rx1 peak should be in here
 
 [pk(3) idx(3)] = max(max(w2a,0));  # Rx1 peak
 if (pk(3) < p.thr1)               # quit if it isn't as large as expected
   return
 endif
 r.pk3 = pk(3);
 
 rx1no = idx(3) - (p.t3 * fsr);  # Rx1 peak negative offset for search start
 w3a = w2a(rx1no:idx(3)+p.zwl*fsr);
 # w3a = w3a ./ max(w3a);   # normalize

 #plot(w3a);  # === DEBUG ===
 #grid on; axis("tight"); ch=kbhit();
 
 idx5 = find(w3a > p.thr1,1)-1; # find 1st data before Rx1 threshold
 w5 = w3a(idx5:end);
 x = linspace(1,length(w5),length(w5));
 zc = zerocrossing(x,w5);  # zc(1) is 1st zero crossing after peak value
 
 idx5a = idx5 - int32(p.tB4 * fsr); # index of search start before pk
 w5a = w3a(idx5a:idx5+1);  # ROI to search for zero-crossing before peak
 x3 = linspace(1,length(w5a),length(w5a));
 zc3 = zerocrossing(x3,w5a);  # zc3(end) is last zerocrossing before peak

  # w1 is full resampled input vector p.w0  w2 is w1 starting at sl2
 r1Off = zc(1) + double(idx5 + rx1no + sl2 - 3);  # index into w1() vector (fsr)
 r1Off2 = zc3(end) + double(idx5a + rx1no + sl2 - 3);
 ...
 # ------------------------------
 rx1Edge = ((r1Off+r1Off2) / (2*fsr)); # rx1 pos'n: avg of two zeros by peak
 rx2Edge = ((rOff+rOff2) / (2*fsr));  # rx2 pos'n: avg of two zeros by peak
 txEdge = ((tOff+tOff2) / (2*fsr));  # tx pos'n
 t1delay = rx1Edge - txEdge;  # <= Result: Elapsed time Tx ... Rx
 ...
 if (0)
  ...
  if (p.doPlot)
   ...
   plot(xr,w1,tOff2,0,"xm",tOff,0,"xg",r1Off2,0,"xb",r1Off,0,"xr",
     rOff2,0,"xb",rOff,0,"xr");
...
# 25.0 C: 346.06 m/s
 ...
 throwCount = 4; # throw away this many pulses at start
...
# p.p=30;  # resampling parameters
 ...
 while (1)
   ...
   # =================================================      
   if (r2.t2delay < 0) # quit due to error?
     return;
   endif

   d1b = (r2.t1delay * vs)/2;  # observed 1-way distance to reflection
   d2b = (r2.t2delay * vs)/2;  # observed 1-way distance to reflection

   TxDiff = r1.txEdge-r2.txEdge;  # time interval between Ch1 and Ch2 (Tx)
   Rx1Diff = r2.rx1Edge-r1.rx1Edge; # interval between Ch1 and Ch2 (Rx1)
   Rx2Diff = r2.rx2Edge-r1.rx2Edge; # interval between Ch1 and Ch2 (Rx1)
 
   if (p.count > throwCount)     
    dat21(di2) = d1b; # measured speed of sound to Refl.2
    dat22(di2) = d2b; # measured speed of sound to Refl.2    
    pk(di2,3) = r2.pk1;  # Ch.2 peak Tx amplitude
    pk(di2,4) = r2.pk2;  # Ch.2 peak Rx amplitude
    CD1(di2) = TxDiff*1E6;  # time interval Ch1..Ch2
    #CD1(di2) = Rx1Diff*1E6;  # time interval Ch1..Ch2
    CD2(di2) = Rx2Diff*1E6;
 ...
 stdev11 = std(dat11)*1E3;   # standard deviation normally < 0.014
 stdev12 = std(dat12)*1E3;   # standard deviation normally < 0.014
 stdev21 = std(dat21)*1E3;   # standard deviation normally < 0.014
 stdev22 = std(dat22)*1E3;   # standard deviation normally < 0.014
 a11 = mean(dat11);
 a12 = mean(dat12);
 a21 = mean(dat21);
 a22 = mean(dat22);
 da1 = mean(CD1);     # time interval between channels on Tx peaks
 da2 = mean(CD2);     # time interval on Rx1 peaks
 p1 = mean(pk(:,1));  # Tx, ch1
 p2 = mean(pk(:,2));  # Rx, ch1
 p3 = mean(pk(:,3));  # Tx, ch2
 
 if (stdev12 > 0.1)    # comment out the results with large variance
   printf("# ");
 endif

 printf("%s ",tstamp);
 printf("%d %8.6f %5.3f %8.6f %5.3f %8.6f %5.3f %8.6f %5.3f %5.3f %5.3f %5.3f",
     di1,a11,stdev11,a12,stdev12,a21,stdev21,a22,stdev22,p1,da1,da2); 
 printf("\n"); # end of output
 return

# di1,a11,stdev11,a12,stdev12,a21,stdev21,a22,stdev22,p1,p2,p3); 
