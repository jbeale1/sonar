...
# v0.71 J.Beale 15-Nov-2020  tuning for Sensor #3
...
#infile='R_1605161384.wav';
#infile='R_1605476991.wav';
...
function [r] = findPks(p)
 ...
 fhp =  2000;     # prefilter: highpass frequency shoulder in Hz (2000)
 ...
 # ---- find Tx-RxFID time
 fr1 = idx(1) + int32(p.t4*fsr);  # fiducial range search start
 fr2 = idx(1) + int32(p.t4a*fsr); # fiducial range search end
                             # Oops: need to check for out-of-range error
 wl = length(w1);
 if (fr1 > wl) || (fr2 > wl)
  return
 endif
 w1a = w1(fr1:fr2);               # data range with fiducial
 [pk2 idx2] = max(max(w1a,0));  # find fiducial peak
 r.fRx = double((idx2 + fr1) - idx(1)) / fsr;  # time delta, Tx to Rx(fiducial)
 # -----
...
#Tback=13.4; # current pipe-back temperature
#Tback=13.0;

#Tback=20.2; # current pipe-back temperature

#SensorOffset = .0386+0.03;  # extra length (m) sensor adds to pipe
SensorOffset = .0386+0.03+.0457;  # extra length (m) sensor adds to pipe
              # 0.049 m added by 2nd coupler on top with cable passthrough
...
# 25.0 C: 346.06 m/s
 ...
 t1 = 0.060;  # seconds width of window, containg Tx, Rx
 ...
 p.t4 = .004167; # seconds after Tx pk to start looking for fiducial
 # p.t4a = .0375;  # seconds after Tx pk to stop looking for fiducial
 p.t4a = .032;  # max sec after Tx pk fiducial might be (2021-01-03: had to move it up)
...
# p.p=30;  # resampling parameters
 ...
 while (1)
   ...
   d1a = (r1.fRx * vs)/2;  # observed 1-way distance to refl.1
   d2a = (r1.t2delay * vs)/2;  # observed 1-way distance to refl.2
   
   
   # printf("%8.6f\n",r1.t2delay);  # DEBUG show raw time delay value

   if (p.count > throwCount)
    dat11(di1) = d1a;    # Ch.1 measured distance
    dat12(di1) = d2a - SensorOffset;    # Ch.1 dist, minus sensor length
 ...
 stdev11 = std(dat11)*1E3;
 stdev12 = std(dat12)*1E3;   # standard deviation normally < 0.014

 a11 = mean(dat11);          # avg of fiducial distance
 a12 = mean(dat12);          # avg of water distance
 p1 = mean(pk(:,1));  # Tx, ch1
 p2 = mean(pk(:,2));  # Rx, ch1
 ...
 if (stdev12 > 20.0)    # comment out the results with large variance (mm)
  ...
  # PulseCnt,WaterDist(m),WStd(mm),FidDist(m),FStd(mm),Amplitude,m/s
 printf("%s ",tstamp);
 printf("%d %8.6f %5.3f %8.6f %5.3f %5.3f %5.1f %5.4f",
     di1-1,a12,stdev12,a11,stdev11,p1, Tback,vs); 
 printf("\n"); # end of output
 return
